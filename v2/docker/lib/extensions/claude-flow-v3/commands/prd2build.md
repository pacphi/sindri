...
if (buildOnly || docAndBuild) {
  ...
  // Step 1: Initialize swarm with hierarchical topology
  Bash("npx @claude-flow/cli@latest swarm init --topology hierarchical --strategy specialized --no-color 2>/dev/null || true")
  ...
  // ============================================================
  // This verification runs AFTER agents report completion

  // DO NOT report build complete until:

  // 1. Check memory for build/milestone value
  //    Bash("npx @claude-flow/cli@latest memory retrieve --key 'build/milestone' --namespace build")
  //    Must equal M3 (final milestone)

  // 2. Check memory for build/status value
  //    Bash("npx @claude-flow/cli@latest memory retrieve --key 'build/status' --namespace build")
  //    Must equal "complete"

  // 3. Verify all milestone completion signals exist in memory:
  //    - m0_complete
  //    - m1_complete
  //    - m2_complete
  //    - m3_complete
  ...
  //    - Continue monitoring memory
  //    - Check every 30 seconds
  //    - DO NOT report completion until all pass

  // 5. When ALL verifications pass:
  //    - Generate final build completion report
  //    - Store final status: memory store --key 'build/completed_at' --value '[timestamp]' --namespace build
  //    - Report completion to user with all milestones' results
```
