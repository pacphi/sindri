...
for ext in $(yq '.extensions | keys | .[]' v2/docker/lib/registry.yaml 2>/dev/null); do
  if [[ ! -f "v2/docker/lib/extensions/$ext/extension.yaml" ]]; then
...
for profile in $(yq '.profiles | keys | .[]' v2/docker/lib/profiles.yaml 2>/dev/null); do
  for ext in $(yq ".profiles.$profile.extensions[]" v2/docker/lib/profiles.yaml 2>/dev/null); do
    if ! yq -e ".extensions.$ext" v2/docker/lib/registry.yaml > /dev/null 2>&1; then
...
for ext in $(yq '.extensions | keys | .[]' v2/docker/lib/registry.yaml 2>/dev/null); do
  category=$(yq ".extensions.$ext.category" v2/docker/lib/registry.yaml 2>/dev/null)
  if [[ -n "$category" ]] && ! yq -e ".categories.$category" v2/docker/lib/categories.yaml > /dev/null 2>&1; then
...
for ext_dir in v2/docker/lib/extensions/*/; do
  ...
  if [[ -f "$ext_file" ]]; then
    ...
    for dep in $deps; do
      if [[ -n "$dep" ]] && [[ ! -d "v2/docker/lib/extensions/$dep" ]]; then
...
for example in $(find examples -name "*.sindri.yaml" 2>/dev/null || true); do
  ...
  if [[ -n "$profile" ]] && ! yq -e ".profiles.$profile" v2/docker/lib/profiles.yaml > /dev/null 2>&1; then
...
for template in $(yq '.templates | keys | .[]' v2/docker/lib/project-templates.yaml 2>/dev/null); do
  for ext in $(yq ".templates.$template.extensions[]" v2/docker/lib/project-templates.yaml 2>/dev/null); do
    if ! yq -e ".extensions.$ext" v2/docker/lib/registry.yaml > /dev/null 2>&1; then
