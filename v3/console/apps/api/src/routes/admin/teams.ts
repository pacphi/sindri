...
router.get('/', rateLimitDefault, async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: 'Validation Error', message: 'Invalid query parameters', details: parsed.error.flatten() }, 422);
  }
...
router.post('/:id/members', rateLimitStrict, async (c) => {
  ...
  try {
    const member = await addTeamMember(teamId, { user_id: parsed.data.userId, role: parsed.data.role as 'ADMIN' | 'OPERATOR' | 'DEVELOPER' | 'VIEWER' | undefined });
    const auth = c.get('auth');
    await createAuditLog({
      user_id: auth.userId,
      team_id: teamId,
    ...
    return c.json({ teamId: member.team_id, userId: member.user_id, role: member.role, joinedAt: member.joined_at.toISOString() }, 201);
  } catch (err: unknown) {
...
router.get('/:id/instances', rateLimitDefault, async (c) => {
  ...
  try {
    ...
    return c.json({ instances: instances.map((i) => ({ id: i.id, name: i.name, provider: i.provider, status: i.status })) });
  } catch (err) {
...
router.delete('/:id/instances/:instanceId', rateLimitStrict, async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Internal Server Error', message: 'Failed to remove instance from team' }, 500);
  }
});
...
function serializeTeamBasic(team: { id: string; name: string; description: string | null; created_by: string | null; created_at: Date; updated_at: Date }) {
  return {
    id: team.id,
    name: team.name,
...
function serializeTeamDetail(team: ReturnType<typeof serializeTeam> extends infer T ? T & {
  members: Array<{
    user_id: string;
    role: string;
    joined_at: Date;
  ...
  return {
    ...base,
    members: (team as { members: Array<{ user_id: string; role: string; joined_at: Date; user: { id: string; email: string; name: string | null; role: string } }> }).members.map((m) => ({
      userId: m.user_id,
      role: m.role,
