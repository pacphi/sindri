...
const UpdateChannelSchema = z.object({
  ...
  config: z.union([WebhookConfigSchema, SlackConfigSchema, EmailConfigSchema, InAppConfigSchema]).optional(),
...
const ListRulesQuerySchema = z.object({
  ...
  severity: AlertSeverityEnum.optional(),
  enabled: z.string().transform((v) => v === 'true').optional(),
  instanceId: z.string().optional(),
  page: z.coerce.number().int().min(1).default(1),
  pageSize: z.coerce.number().int().min(1).max(100).default(20),
});
...
alertsRouter.get('/', rateLimitDefault, async (c) => {
  const queryResult = ListAlertsQuerySchema.safeParse(Object.fromEntries(new URL(c.req.url).searchParams));
...
alertsRouter.post('/bulk-acknowledge', rateLimitStrict, requireRole('OPERATOR'), async (c) => {
  let body: unknown;
  try { body = await c.req.json(); } catch { return c.json({ error: 'Bad Request', message: 'Invalid JSON' }, 400); }
...
alertsRouter.post('/:id/acknowledge', rateLimitStrict, requireRole('OPERATOR'), async (c) => {
  ...
  if (!alert) return c.json({ error: 'Not Found', message: 'Alert not found or already resolved' }, 404);
  return c.json(alert);
});
...
alertsRouter.get('/rules', rateLimitDefault, async (c) => {
  const queryResult = ListRulesQuerySchema.safeParse(Object.fromEntries(new URL(c.req.url).searchParams));
  if (!queryResult.success) {
    return c.json({ error: 'Bad Request', details: queryResult.error.flatten() }, 400);
  }
  const q = queryResult.data;
