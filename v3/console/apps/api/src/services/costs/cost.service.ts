...
export async function getCostSummary(
  ...
  const byInstance = Array.from(byInstanceMap.entries())
    .map(([instanceId, v]) => ({ instanceId, instanceName: v.instanceName, totalUsd: Math.round(v.totalUsd * 100) / 100 }))
...
export async function getCostTrends(
  ...
  // Bucket by day
  const buckets: Map<string, { total: number; compute: number; storage: number; network: number }> = new Map()
