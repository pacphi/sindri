...
describe("Config Drift: Detection", () => {
  it("drift report has required fields: id, instance_id, severity, status, items", () => {
    ...
    expect(["DETECTED", "ACKNOWLEDGED", "REMEDIATING", "RESOLVED", "SUPPRESSED"]).toContain(
      report.status,
    );
  ...
  it("no drift when config matches desired state", () => {
    ...
    const hasExtDrift =
      JSON.stringify(desired.extensions.sort()) !== JSON.stringify(actual.extensions.sort());
  ...
  it("drift report overall severity equals highest item severity", () => {
    ...
    const highestSeverity = items.reduce((max, item) => {
      return severityOrder.indexOf(item.severity) < severityOrder.indexOf(max)
        ? item.severity
        : max;
  ...
  function classifyDriftSeverity(driftType: DriftType): DriftSeverity {
    switch (driftType) {
      case "MISSING_EXTENSION":
        return "CRITICAL";
      case "CONFIG_HASH_CHANGE":
        return "HIGH";
      case "EXTENSION_MISMATCH":
        return "HIGH";
      case "VERSION_MISMATCH":
        return "MEDIUM";
      case "EXTRA_EXTENSION":
        return "LOW";
      case "RESOURCE_DRIFT":
        return "MEDIUM";
    }
...
describe("Config Drift: State Machine", () => {
  ...
  it("drift can be SUPPRESSED from any state", () => {
    const report = makeDriftReport({
      status: "SUPPRESSED",
      suppressed_until: "2026-03-01T00:00:00Z",
    });
