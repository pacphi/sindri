...
type OptimizationAction = 'SUSPEND_IDLE' | 'DOWNSIZE' | 'RIGHTSIZE' | 'SWITCH_PROVIDER' | 'REMOVE_UNUSED';
...
function makeRecommendation(overrides: Partial<OptimizationRecommendation> = {}): OptimizationRecommendation {
  return {
    id: 'rec_01',
    instance_id: 'inst_01',
    action: 'SUSPEND_IDLE',
...
describe('Cost Tracking: Calculation', () => {
  ...
  it('total cost is the sum of all category entries', () => {
    const entries: CostEntry[] = [
      makeCostEntry({ category: 'COMPUTE', amount_usd: 10.50 }),
      makeCostEntry({ category: 'STORAGE', amount_usd: 2.30 }),
      makeCostEntry({ category: 'NETWORK', amount_usd: 0.75 }),
      makeCostEntry({ category: 'EGRESS', amount_usd: 1.20 }),
    ];
  ...
  it('hourly cost is averaged from entries in the hour window', () => {
    const entries: CostEntry[] = [
      makeCostEntry({ amount_usd: 1.00, period_start: '2026-02-17T00:00:00Z', period_end: '2026-02-17T00:15:00Z' }),
      makeCostEntry({ amount_usd: 1.25, period_start: '2026-02-17T00:15:00Z', period_end: '2026-02-17T00:30:00Z' }),
      makeCostEntry({ amount_usd: 0.75, period_start: '2026-02-17T00:30:00Z', period_end: '2026-02-17T00:45:00Z' }),
      makeCostEntry({ amount_usd: 1.00, period_start: '2026-02-17T00:45:00Z', period_end: '2026-02-17T01:00:00Z' }),
    ];
    const hourlyTotal = entries.reduce((sum, e) => sum + e.amount_usd, 0);
    expect(hourlyTotal).toBeCloseTo(4.00, 2);
  });

  it('daily cost aggregates all hourly entries for the day', () => {
    // Simulate 24 hourly entries at $4.00/hour
    const hourlyCost = 4.00;
    const hoursInDay = 24;
    const dailyTotal = hourlyCost * hoursInDay;
    expect(dailyTotal).toBe(96.00);
  });

  it('monthly cost is sum of all daily costs in the period', () => {
    const dailyCost = 96.00;
    const daysInMonth = 30;
    const monthlyTotal = dailyCost * daysInMonth;
    expect(monthlyTotal).toBe(2880.00);
  });

  it('cost is attributed per provider', () => {
    const entries: CostEntry[] = [
      makeCostEntry({ provider: 'fly', amount_usd: 50.00 }),
      makeCostEntry({ provider: 'fly', amount_usd: 30.00 }),
      makeCostEntry({ provider: 'docker', amount_usd: 10.00 }),
    ];
    const byProvider = entries.reduce((acc, e) => {
      acc[e.provider] = (acc[e.provider] ?? 0) + e.amount_usd;
      return acc;
    }, {} as Record<string, number>);
    expect(byProvider['fly']).toBe(80.00);
    expect(byProvider['docker']).toBe(10.00);
  });
});
...
describe('Cost Tracking: Anomaly Detection', () => {
  ...
  it('anomaly is detected when actual exceeds expected by more than 50%', () => {
    const expected = 50.00;
    const actual = 80.00; // 60% above expected
    const deviation = ((actual - expected) / expected) * 100;
  ...
  it('no anomaly when spend is within expected range', () => {
    const expected = 50.00;
    const actual = 55.00; // 10% above expected - within tolerance
    const deviation = ((actual - expected) / expected) * 100;
    const isAnomaly = deviation > 50;
...
describe('Cost Tracking: Attribution', () => {
  it('costs are attributed to the instance that incurred them', () => {
    const entries: CostEntry[] = [
      makeCostEntry({ instance_id: 'inst_01', amount_usd: 10.00 }),
      makeCostEntry({ instance_id: 'inst_02', amount_usd: 5.00 }),
      makeCostEntry({ instance_id: 'inst_01', amount_usd: 8.00 }),
    ];
    const inst01Cost = entries
      .filter((e) => e.instance_id === 'inst_01')
      .reduce((sum, e) => sum + e.amount_usd, 0);
    expect(inst01Cost).toBe(18.00);
  });
  ...
  it('team cost is the sum of all instance costs in the team', () => {
    ...
    const allEntries: CostEntry[] = [
      makeCostEntry({ instance_id: 'inst_01', amount_usd: 10.00 }),
      makeCostEntry({ instance_id: 'inst_02', amount_usd: 5.00 }),
      makeCostEntry({ instance_id: 'inst_03', amount_usd: 20.00 }), // different team
    ];
    const teamCost = allEntries
      .filter((e) => teamInstances.includes(e.instance_id))
      .reduce((sum, e) => sum + e.amount_usd, 0);
    expect(teamCost).toBe(15.00);
  });

  it('cost breakdown shows percentage per category', () => {
    const entries: CostEntry[] = [
      makeCostEntry({ category: 'COMPUTE', amount_usd: 70.00 }),
      makeCostEntry({ category: 'STORAGE', amount_usd: 20.00 }),
      makeCostEntry({ category: 'NETWORK', amount_usd: 10.00 }),
    ];
    const total = entries.reduce((sum, e) => sum + e.amount_usd, 0);
  ...
  it('top spenders are ranked by total cost descending', () => {
    const instanceCosts: { instance_id: string; total_usd: number }[] = [
      { instance_id: 'inst_01', total_usd: 250.00 },
      { instance_id: 'inst_02', total_usd: 500.00 },
      { instance_id: 'inst_03', total_usd: 100.00 },
    ];
...
describe('Cost Tracking: Optimization Recommendations', () => {
  it('recommendation has required fields: instance_id, action, potential_savings_usd', () => {
    ...
    expect(['SUSPEND_IDLE', 'DOWNSIZE', 'RIGHTSIZE', 'SWITCH_PROVIDER', 'REMOVE_UNUSED']).toContain(rec.action);
  ...
  it('recommendations are ranked by potential_savings_usd descending', () => {
    const recs: OptimizationRecommendation[] = [
      makeRecommendation({ id: 'r1', potential_savings_usd: 30.00 }),
      makeRecommendation({ id: 'r2', potential_savings_usd: 120.00 }),
      makeRecommendation({ id: 'r3', potential_savings_usd: 60.00 }),
    ];
  ...
  it('SWITCH_PROVIDER recommendation suggests lower cost alternative', () => {
    ...
    expect(rec.potential_savings_usd).toBe(50.00);
  });

  it('total potential savings is sum of all recommendation savings', () => {
    const recs: OptimizationRecommendation[] = [
      makeRecommendation({ potential_savings_usd: 45.00 }),
      makeRecommendation({ potential_savings_usd: 30.00 }),
      makeRecommendation({ potential_savings_usd: 15.00 }),
    ];
    const totalSavings = recs.reduce((sum, r) => sum + r.potential_savings_usd, 0);
    expect(totalSavings).toBe(90.00);
  });

  it('all optimization action types are recognized', () => {
    const actions: OptimizationAction[] = [
      'SUSPEND_IDLE', 'DOWNSIZE', 'RIGHTSIZE', 'SWITCH_PROVIDER', 'REMOVE_UNUSED',
    ];
