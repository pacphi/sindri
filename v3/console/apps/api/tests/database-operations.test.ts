...
import type { Instance, User, ApiKey } from '@prisma/client'
...
describe('Instance model', () => {
  ...
  it('enforces unique name constraint', async () => {
    ...
    await expect(
      createTestInstance({ name: 'unique-instance' }),
    ).rejects.toThrow()
  })
...
describe('Event model', () => {
  ...
  it('records all supported event types', async () => {
    const instance = await createTestInstance()
    const eventTypes = [
      'DEPLOY', 'REDEPLOY', 'CONNECT', 'DISCONNECT',
      'BACKUP', 'RESTORE', 'DESTROY', 'EXTENSION_INSTALL',
      'EXTENSION_REMOVE', 'HEARTBEAT_LOST', 'HEARTBEAT_RECOVERED', 'ERROR',
    ] as const
