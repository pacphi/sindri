...
const mockFleet: FleetInstance[] = [
  {
    ...
    memPercent: 64.1,
    diskPercent: 45.0,
    lastHeartbeatAt: new Date(Date.now() - 20_000).toISOString(),
    uptimeSeconds: 86400,
  },
  {
    id: "inst_02",
  ...
  },
  {
    ...
    memPercent: 95.0,
    diskPercent: 88.0,
    lastHeartbeatAt: new Date(Date.now() - 400_000).toISOString(),
    uptimeSeconds: 7200,
  },
];
...
describe("Fleet Dashboard: Instance Sorting", () => {
  ...
  it("sort by lastHeartbeatAt descending shows most recent first", () => {
    ...
    const sorted = [...withHeartbeat].sort(
      (a, b) => new Date(b.lastHeartbeatAt!).getTime() - new Date(a.lastHeartbeatAt!).getTime(),
    );
...
describe("Fleet Dashboard: Instance Filtering", () => {
  ...
  it("combined status + provider filter narrows results", () => {
    const filtered = mockFleet.filter((i) => i.status === "RUNNING" && i.provider === "fly");
