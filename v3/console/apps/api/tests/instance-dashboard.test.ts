...
describe('Instance Dashboard: Resource Threshold Alerts', () => {
  ...
  it('multiple simultaneous alerts are displayed as a list', () => {
    const alerts: ThresholdAlert[] = [
      { metric: 'cpu_percent', threshold: 80, currentValue: 92, severity: 'critical', triggeredAt: new Date().toISOString() },
      { metric: 'disk_percent', threshold: 85, currentValue: 87, severity: 'warning', triggeredAt: new Date().toISOString() },
    ];
...
describe('Instance Dashboard: Event Timeline', () => {
  type EventType = 'DEPLOY' | 'REDEPLOY' | 'CONNECT' | 'DISCONNECT' | 'BACKUP' | 'SUSPEND' | 'RESUME' | 'ERROR';
  ...
  const mockEvents: TimelineEvent[] = [
    { id: 'ev_01', eventType: 'DEPLOY', timestamp: '2026-02-17T08:00:00Z', metadata: { version: '1.0.0' } },
    { id: 'ev_02', eventType: 'CONNECT', timestamp: '2026-02-17T08:01:00Z', metadata: { userId: 'user_01' } },
    { id: 'ev_03', eventType: 'BACKUP', timestamp: '2026-02-17T09:00:00Z', metadata: { backupId: 'bkp_001' } },
  ...
  it('event types cover all defined EventType values', () => {
    const types: EventType[] = ['DEPLOY', 'REDEPLOY', 'CONNECT', 'DISCONNECT', 'BACKUP', 'SUSPEND', 'RESUME', 'ERROR'];
...
describe('Instance Dashboard: Multi-Metric Comparison', () => {
  ...
  it('CPU and memory can be overlaid on the same time axis', () => {
    ...
    const memValues = series.map((p) =>
      (Number(p.memUsed) / Number(p.memTotal)) * 100,
    );
  ...
  it('each metric has a unique key for chart series identification', () => {
    const metricKeys = ['cpu_percent', 'mem_percent', 'disk_percent', 'net_bytes_recv', 'net_bytes_sent'];
