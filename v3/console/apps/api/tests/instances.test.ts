...
describe('POST /api/v1/instances', () => {
  ...
  it('registers a new instance and returns 201', async () => {
    ...
    const body = await res.json() as { id: string; name: string; provider: string; status: string };
...
describe('GET /api/v1/instances', () => {
  ...
  it('serializes instance fields correctly', async () => {
    const res = await app.request('/api/v1/instances', { headers: authHeaders() });
    const body = await res.json() as { instances: Array<{ id: string; name: string; status: string; createdAt: string; updatedAt: string }> };
...
describe('GET /api/v1/instances/:id', () => {
  ...
  it('serializes bigint heartbeat fields as strings', async () => {
    ...
    const body = await res.json() as { lastHeartbeat: { memoryUsedBytes: string; memoryTotalBytes: string } };
...
describe('Health endpoint', () => {
  ...
  it('GET /health returns 200 with status ok', async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json() as { status: string; checks: { database: { status: string }; redis: { status: string } } };
