...
type VulnerabilityStatus = 'OPEN' | 'ACKNOWLEDGED' | 'PATCHING' | 'FIXED' | 'ACCEPTED_RISK' | 'FALSE_POSITIVE';
...
describe('Security Dashboard: CVE Detection', () => {
  it('CVE has required fields: cve_id, cvss_score, severity, affected_component, status', () => {
    ...
    expect(['OPEN', 'ACKNOWLEDGED', 'PATCHING', 'FIXED', 'ACCEPTED_RISK', 'FALSE_POSITIVE']).toContain(cve.status);
  });
  ...
  it('CVE dashboard shows open vulnerability counts by severity', () => {
    ...
    const bySeverity = openCves.reduce((acc, c) => {
...
describe('Security Dashboard: Secrets Scanning', () => {
  it('secret finding has required fields: instance_id, secret_type, location, status', () => {
    ...
    expect(['API_KEY', 'TOKEN', 'PASSWORD', 'CERTIFICATE', 'SSH_KEY', 'GENERIC']).toContain(finding.secret_type);
  ...
  it('all supported secret types are recognized', () => {
    const types: SecretType[] = ['API_KEY', 'TOKEN', 'PASSWORD', 'CERTIFICATE', 'SSH_KEY', 'GENERIC'];
...
describe('Security Dashboard: Fleet Summary', () => {
  const instanceScores: SecurityScore[] = [
    makeSecurityScore({ instance_id: 'i1', score: 95, grade: 'A', open_cves: 0, critical_cves: 0, secrets_detected: 0 }),
    makeSecurityScore({ instance_id: 'i2', score: 70, grade: 'C', open_cves: 3, critical_cves: 0, secrets_detected: 1 }),
    makeSecurityScore({ instance_id: 'i3', score: 40, grade: 'F', open_cves: 8, critical_cves: 2, secrets_detected: 2 }),
    makeSecurityScore({ instance_id: 'i4', score: 82, grade: 'B', open_cves: 1, critical_cves: 0, secrets_detected: 0 }),
  ];
