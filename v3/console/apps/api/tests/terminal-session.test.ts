...
describe('Web Terminal PTY Session Lifecycle', () => {
  ...
  describe('Session creation', () => {
    it('creates a terminal session and relays it to the agent', async () => {
      ...
      clientWs.send(
        JSON.stringify(
          makeEnvelope<TerminalCreatePayload>('terminal', 'terminal:create', {
            sessionId,
            cols: 80,
            rows: 24,
          }, { correlationId }),
      ...
      agentWs.send(
        JSON.stringify(
          makeEnvelope<TerminalCreatedPayload>('terminal', 'terminal:created', {
            sessionId,
            pid: 12345,
    ...
    it('rejects terminal session for non-existent instance', async () => {
      const clientWs = await wsConnect(
        `${WS_BASE}/ws/terminal/nonexistent-instance-xyz`,
        { Authorization: `Bearer test-user-token` },
      )
    ...
    it('rejects terminal session without authentication', async () => {
      const connectUnauthenticated = () =>
        wsConnect(`${WS_BASE}/ws/terminal/${INSTANCE_ID}`, {})
  ...
  describe('Data relay', () => {
    it('relays terminal data from client to agent', async () => {
      ...
      const dataMsg = agentMessages.find(
        (m) => m.type === 'terminal:data' && (m.data as TerminalDataPayload).sessionId === sessionId,
      )
