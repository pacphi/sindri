...
export const extensionsApi = {
  ...
  getDependencies(id: string): Promise<{ extension_id: string; dependencies: string[] }> {
    return apiFetch<{ extension_id: string; dependencies: string[] }>(
  ...
  getUsageMatrix(params?: {
    instanceIds?: string[];
    extensionIds?: string[];
  }): Promise<UsageMatrix> {
    const searchParams = new URLSearchParams();
    if (params?.instanceIds?.length) searchParams.set("instanceIds", params.instanceIds.join(","));
    if (params?.extensionIds?.length)
      searchParams.set("extensionIds", params.extensionIds.join(","));
  ...
  listPolicies(
    extensionId?: string,
    instanceId?: string,
  ): Promise<{ policies: ExtensionPolicy[] }> {
    const params = new URLSearchParams();
  ...
  getEffectivePolicies(instanceId: string): Promise<{
    instance_id: string;
