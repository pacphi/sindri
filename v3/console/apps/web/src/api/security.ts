...
export const securityApi = {
  ...
  listSecrets(instanceId?: string, overdueOnly = false): Promise<{ secrets: SecretRotation[]; count: number }> {
    const params = new URLSearchParams()
    if (instanceId) params.set('instanceId', instanceId)
  ...
  listSshKeys(instanceId?: string, status?: string): Promise<{ keys: SshKey[]; summary: SshAuditSummary; count: number }> {
    const params = new URLSearchParams()
    if (instanceId) params.set('instanceId', instanceId)
