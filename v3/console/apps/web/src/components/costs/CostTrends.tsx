            ...
            {activeTab === 'breakdown' && (
              <ResponsiveContainer width="100%" height={224}>
                <AreaChart data={stackedData} margin={{ top: 4, right: 8, left: -8, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="compute" name="Compute" stackId="1"
                    stroke={COST_COLORS.compute} fill={`url(#grad-compute)`} strokeWidth={2} dot={false} isAnimationActive={false} />
                  <Area type="monotone" dataKey="storage" name="Storage" stackId="1"
                    stroke={COST_COLORS.storage} fill={`url(#grad-storage)`} strokeWidth={2} dot={false} isAnimationActive={false} />
                  <Area type="monotone" dataKey="network" name="Network" stackId="1"
                    stroke={COST_COLORS.network} fill={`url(#grad-network)`} strokeWidth={2} dot={false} isAnimationActive={false} />
                </AreaChart>
              </ResponsiveContainer>
            )}
            {activeTab === 'provider' && (
                      ...
                      content={({ active, payload }) => {
                        ...
                        return (
                          <div className="rounded-lg border bg-background px-3 py-2 shadow-md text-xs">
                            ...
                            <p style={{ color: entry.payload.fill as string }}>{formatUsd(entry.value as number)}</p>
                          </div>
                        )
                      }}
                    />
                    ...
                    return (
                      <div key={entry.provider} className="flex items-center gap-2 text-xs">
                        <span
                          ...
                          style={{ background: PROVIDER_COLORS[entry.provider] ?? PROVIDER_COLORS.other }}
                        />
                        <span className="capitalize flex-1 text-muted-foreground">{entry.provider}</span>
