...
function parseSimpleYaml(yaml: string): { value: ParsedSindriConfig | null; parseError: string | null } {
  try {
...
function validateSindriConfig(config: ParsedSindriConfig, yamlLines: string[]): ValidationError[] {
  ...
  if (!config.name) {
    ...
    errors.push({
      severity: 'error',
      message: 'Field "name" must start with a lowercase letter and contain only lowercase letters, digits, and hyphens',
      path: 'name',
      line: findLine('name'),
    })
  }
  ...
  if (!config.deployment) {
    ...
    const validProviders = ['fly', 'kubernetes', 'docker-compose', 'docker', 'devpod', 'e2b', 'runpod', 'northflank']
  ...
  if (!config.extensions) {
    ...
    if (hasActive && ext.additional) {
      errors.push({
        severity: 'error',
        message: 'Field "extensions.additional" cannot be used with "extensions.active" (only with profile)',
...
export function useYamlValidation(yaml: string, options: YamlValidationOptions = {}): ValidationResult {
