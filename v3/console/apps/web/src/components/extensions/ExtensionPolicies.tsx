...
const POLICY_INFO: Record<
  ...
  { label: string; description: string; icon: React.ReactNode; colorClass: string }
> = {
  AUTO_UPDATE: {
    label: "Auto Update",
      ...
      <div className="flex flex-col gap-3">
        ...
        {!instanceId && (
          <div className="flex flex-col gap-1">
            <label className="text-xs text-gray-400">
              Instance ID (leave empty for global policy)
            </label>
            <Input
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, policy: e.target.value as ExtensionUpdatePolicy }))
            }
          >
          ...
          <p className="text-xs text-gray-500">
            {POLICY_INFO[form.policy ?? "AUTO_UPDATE"].description}
          </p>
        </div>
        {form.policy === "PIN" && (
          <div className="flex flex-col gap-1">
    ...
    <div className="flex flex-col gap-5">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-white/10 py-10">
          ...
          <p className="mt-0.5 text-xs text-gray-600">Extensions use AUTO_UPDATE by default</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {data.map((policy) => {
