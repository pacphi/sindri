...
export function NetworkExposure({ instanceName, ports = DEFAULT_PORTS, loading }: Props) {
  if (loading) {
    return (
      <Card>
        <CardHeader><CardTitle>Network Exposure</CardTitle></CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        {instanceName && (
          <p className="text-xs text-muted-foreground mt-1">{instanceName}</p>
        )}
      </CardHeader>
      <CardContent>
        <div className="space-y-1.5">
          ...
          {ports.map((port) => (
            ...
            >
              <span className="font-mono text-xs w-12">{port.port}/{port.protocol}</span>
              <span className="text-sm">{port.service}</span>
              <span className={`text-xs font-medium ${statusColor(port.status)}`}>{port.status}</span>
              <Badge variant={portRiskVariant(port.risk)} className="text-xs">{port.risk}</Badge>
            </div>
          ))}
        </div>
      </CardContent>
