...
```bash
# Contains infra versions, port mappings, credentials, and API service config.
cp .env.example .env
```

Edit `.env` and replace the placeholder secrets with values generated by:

```bash
openssl rand -hex 32
```

**For local pnpm dev (infrastructure via Docker, API/web via pnpm):**

```bash
# Contains localhost DATABASE_URL, REDIS_URL, and all API service config.
cp apps/api/.env.example apps/api/.env
```

Edit `apps/api/.env` and replace the placeholder secrets with values generated by:
...
├── .env.example                # Template — copy to .env (Docker Compose)
├── .env                        # Docker Compose config + API secrets (gitignored)
├── .dockerignore
└── apps/
    └── api/
        ├── .env.example        # Template — copy to apps/api/.env (pnpm dev)
        └── .env                # Local pnpm dev config (gitignored)
```
...
variables into `docker-compose.yml`. Contains infrastructure config (image versions,
port mappings, DB credentials) **and** all API service variables (JWT_SECRET,
NODE_ENV, etc.). The `api` container receives these via an inline `environment:`
block — no separate `apps/api/.env.docker` file is needed.
...
**`apps/api/.env`** — used only for local pnpm dev (when running the API directly
with `pnpm --filter=api run start:dev`). Contains localhost DATABASE_URL, REDIS_URL,
and all API service variables. **This file is not read by Docker Compose** — if you
are using `docker compose up`, you do not need to create it.
...
Edit the placeholder secrets in whichever file applies to your workflow before running.
...
If healthy, verify the `DATABASE_URL` for your workflow:

- **Docker Compose**: `DATABASE_URL` is constructed automatically from the `POSTGRES_*` vars in root `.env`
- **Local pnpm dev**: check `DATABASE_URL` in `apps/api/.env` uses `localhost` as the host
