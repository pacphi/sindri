...
use crate::cli::{
    ...
    ExtensionInstallArgs, ExtensionListArgs, ExtensionLogArgs, ExtensionRemoveArgs,
    ExtensionRollbackArgs, ExtensionStatusArgs, ExtensionUpgradeArgs, ExtensionValidateArgs,
    ExtensionVerifyArgs, ExtensionVersionsArgs, UpdateSupportFilesArgs,
};
use crate::output;
...
pub async fn run(cmd: ExtensionCommands) -> Result<()> {
    match cmd {
        ...
        ExtensionCommands::Log(args) => log(args).await,
    }
}
...
async fn log(args: ExtensionLogArgs) -> Result<()> {
    let ledger = StatusLedger::load_default().context("Failed to load status ledger")?;
