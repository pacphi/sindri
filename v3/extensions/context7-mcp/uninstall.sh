...
if command -v claude &>/dev/null; then
    if claude mcp remove --scope user "${MCP_SERVER_NAME}" 2>/dev/null; then
        print_success "Context7 MCP removed from user scope"
    else
        print_warning "Could not remove Context7 MCP from Claude Code config"
...
print_success "context7-mcp uninstalled"
