...
| **F-AUTHOR-01** | critical | ✅ fixed (Phase 6) | New ADR-030 + `docs/script-contract.md` define the canonical lifecycle hook contract: 8 phases × `ScriptRef { sh, ps1 }`, contracted env (`SINDRI_PHASE`, `SINDRI_COMPONENT_VERSION`, `SINDRI_PRIOR_VERSION`, `SINDRI_TARGET`, `SINDRI_LOG_DIR`, `SINDRI_EVENTS`, `SINDRI_DRY_RUN`), contracted argv `[<phase>, <version>, <prior>]`, binary exit codes, and `$SINDRI_EVENTS` JSON-Lines protocol. New `HooksConfig` shape in `sindri-core::component`; `HooksExecutor::run_phase` dispatcher with contract gate (file present + +x on POSIX); helper libraries at `support/scripts/sindri-helpers.{sh,psm1}`; three new lint warnings (`LINT_HOOK_MISSING_SHEBANG`, `LINT_HOOK_NON_EXECUTABLE`, `LINT_HOOK_MISSING_HELPERS_SOURCE`). ADR-024 superseded; AUTHORING.md rewritten. Industry-practices research: `docs/research/2026-05-04-phase6-lifecycle-research.md`. |
